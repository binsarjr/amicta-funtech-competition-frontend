		...
		<div class="bg-white rounded-lg py-[20px] px-[22px] shadow-lg block w-full" data-cy="card-day">
			...
			<div>
				{#if jadwalMatkul.length}
					<p class="text-[#D9019C] text-[12px]" data-cy="card-desc-{title}">
						{jadwalMatkul.length} Mata Kuliah
					</p>
				{:else}
					<p class="text-gray-500 text-[12px]" data-cy="card-desc-{title}">Belum ada mata kuliah</p>
				{/if}
			</div>
		</div>
